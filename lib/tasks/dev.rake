namespace :dev do
  ...
  task :fake => [:fake_users, :fake_orders]
  ...
  task :fake_users => :environment do
    10.times do
      User.create!( :name => Faker::Internet.email.split("@").first, :email => Faker::Internet.email, :password => "111111", password_confirmation: '111111', is_admin: 'false')
    end
  end
